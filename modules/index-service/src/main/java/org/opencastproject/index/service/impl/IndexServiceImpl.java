...
public class IndexServiceImpl implements IndexService {
  ...
  public Map<String, Map<String, String>> getEventWorkflowProperties(final List<String> eventIds) {
    ...
    final Map<String, Map<String, String>> result = new HashMap<>(eventIds.size());
    for (final ARecord record : r.getRecords().toList()) {
      ...
      for (final Property p : recordProps) {
        eventMap.put(p.getId().getName(), p.getValue().get(Value.STRING));
      }
      final String eventId = record.getMediaPackageId();
      result.put(eventId, eventMap);
    }
    return result;
  }
